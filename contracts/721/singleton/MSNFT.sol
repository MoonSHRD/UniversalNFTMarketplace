...
contract MSNFT is ERC721Enumerable, Ownable {
     ...
     */
    event MaterCopyCreated(
        address indexed author,
        uint256 master_id,
        string indexed description,
        string indexed link
    );
    event MasterCopyCreatedHuman(
        address author,
        uint256 indexed master_id,
        string description,
        string link
    ...
    mapping(address => uint256[]) public author_masterids; // can be used to get all objects created by one author
    ...
    struct ItemInfo {
        ...
        RarityType rarity;
        // @todo rename it
        uint256 i_totalSupply; // 0 means infinite, so it not be actually considered as usual totalSupply(!!)
    ...
    function PlugCrowdSale(
        ...
        require(
            ...
            "MSNFT: only factory contract can plug crowdsale"
        );
        ...
        require(
            mastersales[_masterId] == address(0),
            "MSNFT: you already have plugged sale "
        );
     ...
     * @return _master_id
     */
    function _reserveMasterId(address _author)
        internal
        returns (uint256 _master_id)
    {
     ...
     */
    function createMasterCopy(
        string memory link,
        address payable _author,
        string memory _description,
        uint256 _supplyType
    ) public returns (uint256 c_master_id) {
        require(
            msg.sender == factory_address,
            "MSNFT: only factory contract can create mastercopy"
        );

        uint256 mid = _reserveMasterId(_author);
        require(
            links[link] == 0,
            "MSNFT: file with that link already have been tethered"
        );
     ...
     */
    function set_rarity(uint256 _supplyType)
        private
        pure
        returns (RarityType _rarity)
    {
        if (_supplyType == 1) {
    ...
    function Mint(
        ...
        if (meta.rarity == RarityType.Unique) {
            require(
                itemIds[m_master_id].length == 0,
                "MSNFT: MINT: try to mint more than one of Unique Items"
            );
        }
        if (meta.rarity == RarityType.Rare) {
            require(
                itemIds[m_master_id].length < meta.i_totalSupply,
                " MSNFT: MINT: try to mint more than totalSupply of Rare token"
            );
        }
     ...
     */
    function EmitItem(address to, uint256 m_master_id) public {
        ItemInfo memory meta;
        meta = MetaInfo[m_master_id];
        require(
            msg.sender == meta.author,
            "MSNFT: only author can emit items outside of sale"
        );
    ...
    function buyItem(
        ...
        require(
            _sale == msg.sender,
            "MSNFT: you should call buyItem from itemsale contract"
        );
    ...
    */
     ...
     */
    function get_author(uint256 _masterId)
        public
        view
        returns (address payable _author)
    {
        _author = authors[_masterId];
        return _author;
    }

    function get_master_id_by_link(string memory link_)
        public
        view
        returns (uint256 _masterId)
    {
        _masterId = links[link_];
        return _masterId;
    }

    function get_author_by_link(string memory link_)
        public
        view
        returns (address author_)
    {
     ...
     */
    function getMasterIdByAuthor(address _creator)
        public
        view
     ...
     */
    function getInfobyItemId(uint256 item_id)
        public
        view
        returns (ItemInfo memory)
    {
        uint256 master_id = ItemToMaster[item_id];
